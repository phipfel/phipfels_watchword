...
class WatchwordControllerTest extends \TYPO3\CMS\Core\Tests\UnitTestCase {
	 ...
	 * @test
	 */
	public function listActionFetchesAllWatchwordsFromRepositoryAndAssignsThemToView() {

		$allWatchwords = $this->getMock('TYPO3\\CMS\\Extbase\\Persistence\\ObjectStorage', array(), array(), '', FALSE);
		...
		$watchwordRepository->expects($this->once())->method('findAll')->will($this->returnValue($allWatchwords));
		$this->inject($this->subject, 'watchwordRepository', $watchwordRepository);

		$view = $this->getMock('TYPO3\\CMS\\Extbase\\Mvc\\View\\ViewInterface');
		$view->expects($this->once())->method('assign')->with('watchwords', $allWatchwords);
